!function(b) {
	var a = function(k, s, i) {
		...
		var d = '<div class="daterangepicker dropdown-menu"><div class="calendar left"></div><div class="calendar right"></div><div class="ranges"><div class="range_inputs"><div class="daterangepicker_start_input" style="float: left"><label for="daterangepicker_start">'
		...
		this.container.find(".ranges").on("click", "button.clearBtn",
				b.proxy(this.clickClear, this));
	...
	a.prototype = {
		...
		notify : function() {
			...
			if (this.element.is("input")) {
				this.element.val(this.cleared ? "" : this.startDate
						.format(this.format)
						+ this.separator + this.endDate.format(this.format))
			}
			var d = (this.cleared ? null : this.startDate), c = (this.cleared ? null
					: this.endDate);
			this.cleared = false;
			this.cb(d, c)
		},
		...
		updateCalendars : function() {
			...
			for ( var d in this.ranges) {
				if (this.startDate.isSame(this.ranges[d][0])
					...
					c = false;
					this.container.find(".ranges li:eq(" + e + ")").addClass(
							"active")
				}
				e++
			}
		...
		buildCalendar : function(m, o, n) {
			...
			for ( var g = 0, d = 0, q = 0; g < 42; g++, d++, f = moment(f).add(
					"day", 1)) {
				if (g > 0 && d % 7 == 0) {
					d = 0;
					q++
		...
		renderCalendar : function(h, g, f, c) {
			...
			var d = this.locale.monthNames[h[1][1].month()]
					+ h[1][1].format(" YYYY");
