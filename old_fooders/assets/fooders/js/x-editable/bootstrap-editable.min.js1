...
(function(b) {
	...
	a.prototype = {
		...
		render : function() {
			...
			b.when(this.input.render()).then(
					b.proxy(function() {
						if (!this.options.showbuttons) {
							this.input.autosubmit()
						}
						this.$form.find(".editable-cancel").click(
								b.proxy(this.cancel, this));
						if (this.input.error) {
							this.error(this.input.error);
							this.$form.find(".editable-submit").attr(
									"disabled", true);
							...
							this.$form.find(".editable-submit").removeAttr(
									"disabled");
		...
		error : function(g) {
			var e = this.$form.find(".control-group"), f = this.$form
					.find(".editable-error-block"), c;
			if (g === false) {
		...
		submit : function(g) {
			...
			if (!this.options.savenochange
					&& this.input.value2str(f) == this.input
							.value2str(this.value)) {
					...
					.done(
							b
									.proxy(
											function(e) {
												this.isSaving = false;
												var h = typeof this.options.success === "function" ? this.options.success
														.call(
																this.options.scope,
																e, f)
														: null;
							...
							b.proxy(function(h) {
								...
								if (typeof this.options.error === "function") {
									e = this.options.error.call(
											this.options.scope, h, f)
								} else {
									e = typeof h === "string" ? h
											: h.responseText || h.statusText
													|| "Unknown error!"
								}
								this.error(e);
								this.showForm()
		...
		save : function(d) {
			this.options.pk = b.fn.editableutils.tryParseJson(this.options.pk,
					true);
			var c = (typeof this.options.pk === "function") ? this.options.pk
					.call(this.options.scope) : this.options.pk, e = !!(typeof this.options.url === "function" || (this.options.url && ((this.options.send === "always") || (this.options.send === "auto"
					&& c !== null && c !== undefined)))), f;
			if (e) {
				this.showLoading();
				f = {
				...
				if (typeof this.options.params === "function") {
					...
					this.options.params = b.fn.editableutils.tryParseJson(
	...
	b.fn.editableform = function(d) {
		...
		return this
				.each(function() {
					var g = b(this), f = g.data("editableform"), e = typeof d === "object"
							&& d;
					if (!f) {
	...
	b.fn.editableform.template = '<form method="POST" action="" class="form-inline editableform"><div class="control-group"><div><div class="editable-input"></div><div class="editable-buttons"></div></div><div class="editable-error-block"></div></div></form>';
	b.fn.editableform.loading = '<div class="editableform-loading"></div>';
	b.fn.editableform.buttons = '<button type="submit" name="editablesubmit" class="editable-submit">ok</button><button type="button" class="editable-cancel">cancel</button>';
...
(function(a) {
	a.fn.editableutils = {
		...
		getConfigData : function(b) {
			...
			a
					.each(
							...
							function(e, d) {
								if (typeof d !== "object"
										|| (d && typeof d === "object" && (d.constructor === Object || d.constructor === Array))) {
									c[e] = d
								}
							});
			return c
		...
		supportsTransitions : function() {
			var c = document.body || document.documentElement, f = c.style, g = "transition", d = [
...
(function(c) {
				...
				c(document)
						.on(
								...
								function(j) {
									...
									if (!c.contains(document.documentElement,
											j.target)) {
										return
									}
									if (f.is(document)) {
	...
	c.fn.editableContainer = function(e) {
		...
		return this
				.each(function() {
					var h = c(this), j = "editableContainer", g = h.data(j), f = typeof e === "object"
							&& e, i = (f.mode === "inline") ? a : b;
					if (!g) {
...
(function(a) {
	a.extend(a.fn.editableContainer.Inline.prototype,
...
(function(b) {
	var a = function(d, c) {
		this.$element = b(d);
		this.options = b.extend({}, b.fn.editable.defaults, c,
				b.fn.editableutils.getConfigData(this.$element));
	...
	a.prototype = {
		...
		init : function() {
			...
			if (this.options.value === undefined || this.options.value === null) {
				this.value = this.input
						.html2value(b.trim(this.$element.html()));
				c = true
			} else {
				this.options.value = b.fn.editableutils.tryParseJson(
			...
			if (this.options.toggle !== "manual") {
				...
				this.$element.on(this.options.toggle + ".editable", b.proxy(
						function(g) {
			...
			case "always":
				...
				break;
			case "auto":
				d = !b.trim(this.$element.text()).length && this.value !== null
						&& this.value !== undefined && !c;
				break;
			default:
				d = false
			}
		...
		initLive : function() {
			...
			this.$element.on(this.options.toggle + ".editable", c, b.proxy(
					function(f) {
						var d = b(f.target);
						if (!d.data("editable")) {
			...
			if (c !== undefined) {
				...
				if (b.trim(this.$element.html()) === "") {
					...
					if (b.trim(this.$element.text()) !== "") {
						...
						this.isEmpty = !this.$element.height()
								|| !this.$element.width()
					}
				}
			}
		...
		save : function(g, h) {
			if (this.options.unsavedclass) {
				...
				d = d
						|| (this.options.savenochange && this.input
								.value2str(this.value) !== this.input
								.value2str(h.newValue));
				if (d) {
		...
		destroy : function() {
			...
			this.$element
					.removeClass("editable editable-open editable-disabled");
		...
		case "submit":
			...
			if (b.isEmptyObject(i)) {
				...
				b.ajax(b.extend({
					...
					data : d,
					type : "POST"
				}, f.ajaxOptions)).success(function(k) {
	...
	b.extend(a.prototype, {
		...
		doPrepend : function() {
			...
			if (!b.isArray(this.prependData)) {
				...
				this.options.prepend = b.fn.editableutils.tryParseJson(
...
(function(b) {
	...
	b.extend(a.prototype, {
		...
		activate : function() {
			if (this.$input.is(":visible")) {
				this.$input.focus();
				b.fn.editableutils.setCursorPosition(this.$input.get(0),
		...
		renderClear : function() {
			if (this.options.clear) {
				...
				this.$input.after(this.$clear).css("padding-right", 24).keyup(
						b.proxy(function(d) {
	...
	a.defaults = b.extend({}, b.fn.editabletypes.abstractinput.defaults, {
		tpl : '<input name="editablecontent" type="text">',
		placeholder : null,
		clear : true
	});
	...
	a.extend(b.prototype,
			{
				...
				value2htmlFinal : function(e, d) {
					var f = "", c = a.fn.editableutils.itemsByValue(e,
							this.sourceData);
					if (c.length) {
						f = c[0].text
					}
	...
	b.extend(a.prototype,
			{
				...
				value2str : function(c) {
					return b.isArray(c) ? c.sort().join(
							b.trim(this.options.separator)) : ""
				},
				str2value : function(e) {
					var c, d = null;
					if (typeof e === "string" && e.length) {
						c = new RegExp("\\s*" + b.trim(this.options.separator)
								+ "\\s*");
						d = e.split(c)
					} else {
						if (b.isArray(e)) {
				...
				value2htmlFinal : function(f, d) {
					var c = [], e = b.fn.editableutils.itemsByValue(f,
							this.sourceData);
					if (e.length) {
...
(function(a) {
	...
	b.defaults = a
			.extend(
					{},
					a.fn.editabletypes.number.defaults,
					{
	...
	a.extend(b.prototype, {
		...
		value2html : function(e, c) {
			...
			if (this.options.select2.tags) {
				...
				if (this.sourceData) {
					d = a.fn.editableutils.itemsByValue(e, this.sourceData,
							"id")
				} else {
				}
			}
		...
		html2value : function(c) {
			return this.options.select2.tags ? this.str2value(c,
		...
		value2input : function(e) {
			if (this.isRemote) {
				var d, c;
				if (this.sourceData) {
					c = a.fn.editableutils.itemsByValue(e, this.sourceData,
							"id");
					if (c.length) {
						d = c[0]
					}
		...
		str2value : function(g, e) {
			...
			e = e || this.options.select2.separator
...
(function(b) {
	var a = function(d, c) {
		...
		this.options = b.extend({}, b.fn.combodate.defaults, c, this.$element
				.data());
		this.init()
	};
	a.prototype = {
		...
		init : function() {
			...
			this.$widget = b('<span class="combodate"></span>').html(
					this.getTemplate());
			this.initCombos();
		...
		getTemplate : function() {
			...
			b.each(this.map, function(e, d) {
				d = d[0];
				var g = new RegExp(d + "+"), f = d.length > 1 ? d.substring(1,
						2) : d;
				c = c.replace(g, "{" + f + "}")
			});
			c = c.replace(/ /g, "&nbsp;");
		...
		initItems : function(d) {
			...
			if (this.options.firstItem === "name") {
				...
				var f = typeof e[d] === "function" ? e[d](1, true, d, false)
						: e[d];
				f = f.split(" ").reverse()[0];
				c.push([ "", f ])
			} else {
		...
		renderItems : function(c) {
			var e = [];
			for ( var d = 0; d < c.length; d++) {
				e.push('<option value="' + c[d][0] + '">' + c[d][1]
						+ "</option>")
			}
			return e.join("\n")
		},
		fillDay : function() {
			var d = this.initItems("d"), e, f, c = this.options.template
					.indexOf("DD") !== -1;
			for (f = 1; f <= 31; f++) {
		...
		fillMonth : function() {
			var d = this.initItems("M"), e, f, h = this.options.template
					.indexOf("MMMM") !== -1, g = this.options.template
					.indexOf("MMM") !== -1, c = this.options.template
					.indexOf("MM") !== -1;
			for (f = 0; f <= 11; f++) {
				if (h) {
		...
		fillHour : function() {
			var e = this.initItems("h"), f, h, k = this.options.template
					.indexOf("h") !== -1, j = this.options.template
					.indexOf("H") !== -1, d = this.options.template
					.toLowerCase().indexOf("hh") !== -1, g = k ? 1 : 0, c = k ? 12
					: 23;
			for (h = g; h <= c; h++) {
		...
		fillMinute : function() {
			var d = this.initItems("m"), e, f, c = this.options.template
					.indexOf("mm") !== -1;
		...
		fillSecond : function() {
			var d = this.initItems("s"), e, f, c = this.options.template
					.indexOf("ss") !== -1;
		...
		fillAmpm : function() {
			var d = this.options.template.indexOf("a") !== -1, e = this.options.template
					.indexOf("A") !== -1, c = [ [ "am", d ? "am" : "AM" ],
					[ "pm", d ? "pm" : "PM" ] ];
			return c
		},
		getValue : function(f) {
		...
		setValue : function(f) {
			...
			var e = typeof f === "string" ? moment(f, this.options.format)
					: moment(f), d = this, c = {};
			function g(h, i) {
				var j = {};
			...
			if (e.isValid()) {
				...
				b.each(c, function(i, h) {
					if (d["$" + i]) {
						if (i === "minute" && d.options.minuteStep > 1
								&& d.options.roundTime) {
							h = g(d["$" + i], h)
						}
						if (i === "second" && d.options.secondStep > 1
								&& d.options.roundTime) {
							h = g(d["$" + i], h)
						}
						d["$" + i].val(h)
		...
		highlight : function(c) {
			if (!c.isValid()) {
				if (this.options.errorClass) {
					...
					if (!this.borderColor) {
						this.borderColor = this.$widget.find("select").css(
								"border-color")
					}
	...
	b.fn.combodate = function(e) {
		...
		c.shift();
		if (e === "getValue" && this.length
				&& (f = this.eq(0).data("combodate"))) {
			return f.getValue.apply(f, c)
		}
		return this.each(function() {
			var h = b(this), g = h.data("combodate"), d = typeof e == "object"
					&& e;
			if (!g) {
...
(function(a) {
	var b = function(c) {
		...
		this.options.combodate = a.extend({}, b.defaults.combodate,
				c.combodate, {
