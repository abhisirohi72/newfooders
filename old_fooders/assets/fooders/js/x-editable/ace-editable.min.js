			...
			.extend(
					a.prototype,
					{
						initImage : function(c, d) {
							...
							this.name = this.options.image.name
						...
						render : function() {
							...
							this.options.image.before_change = this.options.image.before_change
									|| function(f, g) {
										var d = f[0];
										if (typeof d === "string") {
											if (!(/\.(jpe?g|png|gif)$/i)
											...
											var e = b.trim(d.type);
											if ((e.length > 0 && !(/^image\/(jpe?g|png|gif)$/i)
													.test(e))
													|| (e.length == 0 && !(/\.(jpe?g|png|gif)$/i)
															.test(d.name))) {
												if (c.on_error) {
													c.on_error(1)
												}
							...
							this.options.image.before_remove = this.options.image.before_remove
									|| function() {
										c.$input.val(null);
										return true
									};
							this.$file
									.ace_file_input(this.options.image)
									.on(
											"change",
											function() {
												var d = (c.$file.val() || c.$file
														.data("ace_input_files")) ? Math
														.random()
														+ ""
														+ (new Date())
																.getTime()
										...
										width : "150px"
									}).closest(".editable-input").addClass(
											"editable-image")
						}
					});
	a.defaults = b
			.extend(
					{},
					b.fn.editabletypes.abstractinput.defaults,
					{
			...
			this.$tpl.parent().find(".wysiwyg-editor").show().ace_wysiwyg(
					{
						toolbar : [ "bold", "italic", "strikethrough",
								"underline", null, "foreColor", null,
								"insertImage" ]
					}).prev().addClass("wysiwyg-style2").closest(
					".editable-input").addClass("editable-wysiwyg").closest(
					".editable-container").css({
				display : "block"
			});
			if (this.options.wysiwyg && this.options.wysiwyg.css) {
				this.$tpl.closest(".editable-wysiwyg").css(
						this.options.wysiwyg.css)
			}
		},
			...
			.extend(
					a.prototype,
					{
						initSlider : function(c, d) {
							this.options.slider = b.extend({}, d.slider,
									c.slider)
						},
						render : function() {
						},
						activate : function() {
							if (this.$input.is(":visible")) {
								this.$input.focus();
								b.fn.editableutils.setCursorPosition(
										this.$input.get(0),
										this.$input.val().length);
								var c = this;
												...
												this.options.slider,
												{
													...
													slide : function(g, h) {
														...
														if (h.handle.firstChild == null) {
															b(h.handle)
																	.append(
																			"<div class='tooltip top in' style='display:none;top:-42px;left:-4px;'><div class='tooltip-arrow'></div><div class='tooltip-inner'></div></div>")
														}
														b(h.handle.firstChild)
																.show()
																.children().eq(
																		1)
																.text(i)
